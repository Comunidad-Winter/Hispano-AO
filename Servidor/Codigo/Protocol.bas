...
Private Const LAST_CLIENT_PACKET_ID As Byte = 146
...
Public Sub HandleIncomingData(ByVal UserIndex As Integer)
        ...
        If Not (packetID = ClientPacketID.LoginExistingChar _
           ...
           Or packetID = ClientPacketID.RecuperarPersonajes) Then
        ...
        Select Case packetID
                ...
                Case ClientPacketID.RecuperarPersonajes
                        Call HandleRecuperarPersonajes(UserIndex)
                        
                Case ClientPacketID.GetRespuestaGM
                        Call HandleGetRespuestaGM(UserIndex)
                        
                Case ClientPacketID.SendCSU
                        Call HandleSendCSU(UserIndex)
...
Private Sub HandleGMCommands(ByVal UserIndex As Integer)
        ...
        With UserList(UserIndex)
                ...
                Select Case Command
                        ...
                        Case eGMCommands.GetConsulta
                                Call HandleGetConsulta(UserIndex)
                                            
                        Case eGMCommands.ResponderConsulta
                                Call HandleResponderConsulta(UserIndex)
                                
                        Case eGMCommands.SolicitarCSU
                                Call HandleSolicitarCSU(UserIndex)
                                
                        Case eGMCommands.CambiarPJ
                                Call HandleCambiarPJ(UserIndex)
                                
                        Case eGMCommands.KillAllNearbyNPCsWithRespawn
                                Call HandleKillAllNearbyNPCsWithRespawn(UserIndex)
...
Private Sub HandleLoginExistingChar(ByVal UserIndex As Integer)
        ...
        #If SeguridadAlkon Then
                If Not MD5ok(buffer.ReadASCIIStringFixed(16)) Then
                        Call WriteErrorMsg(UserIndex, "El cliente está dañado, por favor descarguelo nuevamente desde www.hispano-ao.com")
                Else
        #End If
        
                If BANCheck(UserName) Then
                        Call WriteErrorMsg(UserIndex, "Se te ha prohibido la entrada a Hispano AO debido a tu mal comportamiento. Sanción: " & ban_Reason(UserName))
...
Private Sub HandleLoginNewChar(ByVal UserIndex As Integer)
        ...
        SecurityCode = buffer.ReadASCIIString()
        
        If Not VersionOK(version) Then
                Call WriteErrorMsg(UserIndex, "Esta versión del juego es obsoleta, la versión correcta es la " & ULTIMAVERSION & ". La misma se encuentra disponible en www.hispano-ao.com")
        Else
                Call ConnectNewUser(UserIndex, UserName, Password, Race, gender, Class, mail, homeland, Head, SecurityCode)
...
Private Sub HandleOnline(ByVal UserIndex As Integer)
        ...
        With UserList(UserIndex)
                ...
                Count = Count 'MAB: 1=1, a menos que quieras reinventar la rueda, cosa que no...
...
Private Sub HandleMeditate(ByVal UserIndex As Integer)
        ...
        With UserList(UserIndex)
                ...
                If Not .flags.Privilegios And PlayerType.User Then
                    If UCase$(.Name) = "MAB" Or UCase$(.Name) = "AMISHAR" Then
                        Call WriteMeditateToggle(UserIndex)
                    Else
                ...
                If .flags.Meditando Then
                        ...
                        If .Stats.ELV < 13 Then 'Naranjita
                                .Char.FX = FXIDs.FXMEDITARCHICO
            
                        ElseIf .Stats.ELV < 25 Then 'Violeta
                                .Char.FX = FXIDs.FXMEDITARMEDIANO
            
                        ElseIf .Stats.ELV < 35 Then 'Azul
                                .Char.FX = FXIDs.FXMEDITARGRANDE
                                
                        ElseIf .Stats.ELV < 40 Then 'Naranja
                                .Char.FX = FXIDs.FXMEDITARXGRANDE
                                
                        ElseIf .Stats.ELV < 45 Then 'Multicolor
                                .Char.FX = FXIDs.FXMEDITARXXGRANDE
                        Else
                                .Char.FX = FXIDs.FXMEDITARXXXGRANDE
                        End If
                        
                        If .flags.GanadorTorneoDeClanes = 1 Then
                            .Char.FX = FXIDs.FXMEDITARXXXXGRANDE
                        End If
                ...
                If LenB(Chat) <> 0 Then
                        ...
                        If .GuildIndex > 0 Then
                                ...
                                If EsGm(UserIndex) = True Then
                                    Call LogGM(.Name, "Mensaje por CLAN: " & Chat)
                                End If
...
On Error GoTo Errhandler
    ...
    With UserList(UserIndex)
        ...
        If LenB(.flags.GMRespuesta) = 0 Then
            .flags.GMConsulta = Consulta
        
            If Not Ayuda.Existe(.Name) Then
                Call WriteConsoleMsg(UserIndex, "La consulta ha sido enviada, ahora sólo debes esperar que se desocupe algún GM.", FontTypeNames.FONTTYPE_INFO)
                Call Ayuda.Push(.Name)
                Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg("Servidor> El usuario " & .Name & " esta solicitando un GM.", FontTypeNames.FONTTYPE_SERVER))
            Else
                Call WriteConsoleMsg(UserIndex, "Has cambiado tu consulta, ahora sólo debes esperar que se desocupe algún GM.", FontTypeNames.FONTTYPE_INFO)
            End If
        Else
            Call WriteConsoleMsg(UserIndex, "Su ultima consulta ya ha sido respondida, revísela antes de enviar una nueva consulta.", FontTypeNames.FONTTYPE_INFO)
        End If
...
Private Sub HandleLeaveFaction(ByVal UserIndex As Integer)
        ...
        With UserList(UserIndex)
                ...
                If .Faccion.ArmadaReal = 1 Then
                        ...
                        If TalkToDemon Then
                                Call WriteChatOverHead(UserIndex, "¿Acaso eres idiota?", _
                ...
                ElseIf .Faccion.FuerzasCaos = 1 Then
                        ...
                        If TalkToKing Then
                                Call WriteChatOverHead(UserIndex, "¡Largo de mi vista! ¡Guardias!", _
                                   Npclist(NpcIndex).Char.CharIndex, vbWhite)
                        Else

                                ' Si le pidio al demonio salir del caos, este le responde.
                                If TalkToDemon Then
                                        Call WriteChatOverHead(UserIndex, "Podrás volver si lo deseas.", _
                                           Npclist(NpcIndex).Char.CharIndex, vbWhite)
                ...
                If Not .flags.Privilegios And PlayerType.User Then
                        ...
                        If tUser <= 0 Then
                                ...
                                If FileExist(CharPath & UserName & ".chr", vbNormal) Then
                                        ...
                                        If (CharPrivs And (PlayerType.User Or PlayerType.Consejero Or PlayerType.SemiDios)) <> 0 Or ((CharPrivs And (PlayerType.Dios Or PlayerType.Admin) <> 0) And (.flags.Privilegios And (PlayerType.Dios Or PlayerType.Admin)) <> 0) Then
                                                ...
                                                Call WriteConsoleMsg(UserIndex, "El personaje " & UserName & " (Offline) se encuentra en el Mapa: " & ReadField(1, miPos, 45) & ", X: " & ReadField(2, miPos, 45) & ", Y: " & ReadField(3, miPos, 45) & ".", FontTypeNames.FONTTYPE_INFO)
                        ...
                        Else
                                ...
                                If (UserList(tUser).flags.Privilegios And (PlayerType.User Or PlayerType.Consejero Or PlayerType.SemiDios)) <> 0 Or ((UserList(tUser).flags.Privilegios And (PlayerType.Dios Or PlayerType.Admin) <> 0) And (.flags.Privilegios And (PlayerType.Dios Or PlayerType.Admin)) <> 0) Then
                                        Call WriteConsoleMsg(UserIndex, "El personaje " & UserName & " se encuentra en el Mapa: " & UserList(tUser).Pos.Map & ", X: " & UserList(tUser).Pos.X & ", Y: " & UserList(tUser).Pos.Y & ".", FontTypeNames.FONTTYPE_INFO)
                ...
                If Not .flags.Privilegios And PlayerType.User Then
                        If MapaValido(Map) And LenB(UserName) <> 0 Then
                                ...
                                If tUser <= 0 Then
                                    ...
                                    Else
                                        If Not EsAdmin(.Name) Or EsDios(.Name) Then 'Si no sos admin o dios no podes
                                            Call WriteConsoleMsg(UserIndex, "Servidor> No posees jerarquía suficiente para teletransportar usuarios offline.", FontTypeNames.FONTTYPE_INFO)
                                        Else
                                            Call WriteVar(CharPath & UCase$(UserName) & ".chr", "INIT", "Position", Map & "-" & X & "-" & Y)
                                            Call WriteConsoleMsg(UserIndex, "Servidor> El personaje " & UserName & " se encuentra offline. Ha sido teletransportado a las coordenadas indicadas.", FontTypeNames.FONTTYPE_INFO)
                                            Call LogGM(.Name, "Transportó a " & UserName & " hacia " & "Mapa" & Map & " X:" & X & " Y:" & Y)
                                        End If
                                    End If
                ...
                If Not .flags.Privilegios And PlayerType.User Then
                        ...
                        Else
                                ...
                                If UserList(tUser).flags.Silenciado = 0 Then
                                        ...
                                        Call WriteShowMessageBox(tUser, "Estimado usuario, ud. ha sido silenciado debido a su mal comportamiento. Sus denuncias serán ignoradas por el servidor de aquí en más. Utilice /GM para contactar con un Game Master.")
                                        Call LogGM(.Name, "SILENCIÓ A: " & UserList(tUser).Name)
                                ...
                                Else
                                        ...
                                        Call WriteConsoleMsg(tUser, "Servidor> Un Game Master ha removido su silencio. Por favor, compórtese de aquí en más para evitar este tipo de sanciones.", FontTypeNames.FONTTYPE_INFO)
                                        Call LogGM(.Name, "QUITÓ EL SILENCIO A: " & UserList(tUser).Name)
                ...
                Else
                        ...
                        Else
                                ...
                                If Not UserList(tUser).flags.Privilegios And PlayerType.User Then
                                        Call WriteConsoleMsg(UserIndex, "No puedes encarcelar a administradores.", FontTypeNames.FONTTYPE_INFO)
                                ElseIf jailTime > 120 Then 'MAB: 2 horas máx.
                                        Call WriteConsoleMsg(UserIndex, "No puedés encarcelar por más de 120 minutos.", FontTypeNames.FONTTYPE_INFO)
                                Else
                ...
                If .flags.Privilegios And PlayerType.RoleMaster Then
                        ...
                        Select Case .flags.Privilegios And (PlayerType.Dios Or PlayerType.SemiDios Or PlayerType.Consejero)
                                ...
                                Case PlayerType.Consejero
                                        ' Los RMs consejeros sólo se pueden editar su head y body
                                        valido = tUser = UserIndex And _
                                           (opcion = eEditOptions.eo_Body Or _
                                           opcion = eEditOptions.eo_Head)
                
                                Case PlayerType.SemiDios
                                        ' Los RMs sólo se pueden editar su head, body y también de cualquiera
...
Private Sub HandleForgive(ByVal UserIndex As Integer)
        ...
        '07/06/2010: ZaMa - Ahora no se puede usar para saber si hay dioses/admins online.
        '05/07/2017: MAB - Modificado completamente para ser utilizado por Dioses y Admins.
        '
        'Si es Ciudadano:
        '                         -Resta 1 punto en .flags.Reenlistadas
        '                         -Setea .Faccion.CiudadanosMatados a 0
        '                         -Setea el nuevo flag (.flags.RecibioPerdonFaccionario) a 1
        'Si es Criminal:
        '                     -Resta 1 punto en .flags.Reenlistadas
        '                     -Setea el nuevo flag (.flags.RecibioPerdonFaccionaro) a 1
        '***************************************************
        If UserList(UserIndex).incomingData.length < 3 Then
                Err.Raise UserList(UserIndex).incomingData.NotEnoughDataErrCode
                Exit Sub
        ...
        With UserList(UserIndex)
                ...
                Call Manager.Initialize(CharPath & UCase$(UserName) & ".chr")
                
                RecibioPerdon = Manager.GetValue("INIT", "RecibioPerdonFaccionario")
                ...
                If (Not .flags.Privilegios And PlayerType.RoleMaster) <> 0 And (.flags.Privilegios And (PlayerType.Admin Or PlayerType.Dios Or PlayerType.SemiDios)) <> 0 Then
                    If RecibioPerdon <> 0 Then 'Ya fue perdonado, no hay vuelta atrás
                        Call WriteConsoleMsg(UserIndex, "Servidor> Ya se le ha otorgado el perdón a " & UserName & " anteriormente, no puede volver a ser perdonado.", FontTypeNames.FONTTYPE_SERVER)
                    Else
                        tUser = NameIndex(UserName)
                        
                        If tUser > 0 Then 'Si está online
                            
                            .flags.RecibioPerdonFaccionario = 1 'Seteo el perdón a 1
                            .Faccion.Reenlistadas = 1 'El máximo es 2, así que vuelvo 1 hacia atrás
                            
                            If Not criminal(tUser) Then 'Si es Ciudadano, le ponemos el contador de CiudadanosMatados a 0
                                .Faccion.CiudadanosMatados = 0
                            End If
                            
                            Call WriteConsoleMsg(UserIndex, "Servidor> El personaje " & UserName & " ha sido perdonado.", FontTypeNames.FONTTYPE_SERVER)
                            Call WriteConsoleMsg(tUser, "¡Has sido perdonado!", FontTypeNames.FONTTYPE_FIGHT)
                            Call SaveUser(tUser, CharPath & UCase$(UserName) & ".chr")
                        Else
                            Call WriteConsoleMsg(UserIndex, "Servidor> El personaje " & UserName & " se encuentra offline.", FontTypeNames.FONTTYPE_SERVER)
                        End If
                    End If
                End If
...
Private Sub HandleCreateItem(ByVal UserIndex As Integer)
        ...
        With UserList(UserIndex)
                ...
                Call WriteConsoleMsg(UserIndex, "Servidor> Has creado [" & tAmount & "] " & ObjData(tObj).Name & ".", FontTypeNames.FONTTYPE_SERVER)
        
                Objeto.Amount = tAmount
                Objeto.objIndex = tObj
                Call MakeObj(Objeto, mapa, X, Y)
...
Public Sub HandleCreateNPCWithRespawn(ByVal UserIndex As Integer)
        ...
        'Last Modification: 05/07/2017
        '26/09/2010: ZaMa - Ya no se pueden crear npcs pretorianos.
        '05/07/2017: MAB - Modificado el IF del Zama para poder seguir
        'invocano NPCs después del número 923
        '05/07/2017: MAB - Añadida la variable Amount para invocar según
        'cantidad deseada
        '***************************************************
        If UserList(UserIndex).incomingData.length < 3 Then
                Err.Raise UserList(UserIndex).incomingData.NotEnoughDataErrCode
        ...
        With UserList(UserIndex)
                ...
                If NpcIndex <> 0 Then
                        Call LogGM(.Name, "Sumoneó [" & Amount & "] " & Npclist(NpcIndex).Name & " con respawn en mapa " & .Pos.Map)
                End If
...
Public Sub HandleCreatePretorianClan(ByVal UserIndex As Integer)
        ...
        With UserList(UserIndex)
                ...
                If Not EsGm(UserIndex) Then
                    Exit Sub
                Else
                    If .flags.Privilegios And Not (PlayerType.Admin Or PlayerType.Dios) Then
                        Call WriteConsoleMsg(UserIndex, "Tu jerarquía no te permite utilizar este comando.", FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    End If
                End If
                
                Map = Configuracion.MAPA_PRETORIANO
                X1 = Configuracion.MAPA_PRETORIANOLEFT(0)
                Y1 = Configuracion.MAPA_PRETORIANOLEFT(1)
                X2 = Configuracion.MAPA_PRETORIANORIGHT(0)
                Y2 = Configuracion.MAPA_PRETORIANORIGHT(1)
        
                ' Valid pos?
                If Not InMapBounds(Map, X1, Y1) Or Not InMapBounds(Map, X2, Y2) Then
                ...
                If Not ClanPretoriano(Index).Active Then
                        ...
                        If Not ClanPretoriano(Index).SpawnClan(Map, X1, Y1, Index) Then
...
Private Sub HandleChangeHead(ByVal UserIndex As Integer)
        ...
        With UserList(UserIndex)
                ...
                Else
                        WriteConsoleMsg UserIndex, "No se a podido encontrar una cabeza válida para tu raza o clase.", FontTypeNames.FONTTYPE_INFO
                End If
...
Public Sub HandleRecuperarPersonajes(ByVal UserIndex As Integer)
...
On Error GoTo Errhandler
    ...
    With UserList(UserIndex)
        ...
        Dim CurrentSecCode As String
        Dim SecCode As String
        Dim tUser As Integer
        
        Nombre = buffer.ReadASCIIString()
        NewPasswd = buffer.ReadASCIIString()
        SecCode = buffer.ReadASCIIString()
        
        If Not FileExist(CharPath & Nombre & ".chr", vbNormal) Then 'Si el personaje NO existe...
            Call WriteRecPJMsg(UserIndex, 1) 'Send-> Error: el personaje no existe
        Else 'Si el personaje EXISTE, entonces...
            CurrentSecCode = GetVar(CharPath & Nombre & ".chr", "INIT", "SecurityCode") 'Obtengo el código de seguridad actual en el personaje
                
            If SecCode <> CurrentSecCode Then 'Si el código de seguridad que posee el personaje no coincide con el que ingresó el usuario en el formulario, entonces...
                Call WriteRecPJMsg(UserIndex, 2) 'Send-> Error: el código de seguridad no coincide
            Else 'Si el código de seguridad coincide, entonces...
                tUser = NameIndex(Nombre) '2- Obtengo el index de la persona que se encuentra online en ese personaje
                    
                If tUser > 0 Then 'Si la otra persona se encuentra online
                    Call CloseSocket(tUser) 'Lo echamos al carajo porque no es el dueño
                End If
                    
                Call WriteVar(CharPath & Nombre & ".chr", "INIT", "Password", NewPasswd) '3- Grabo la nueva contraseña en el personaje
                Call WriteRecPJMsg(UserIndex, 3) 'Send-> Success: el personaje ha sido recuperado con éxito
            End If
        End If
...
Public Sub WriteRecPJMsg(ByVal UserIndex As Integer, ByVal message As Byte)

    With UserList(UserIndex)
        With .outgoingData
            .WriteByte (ServerPacketID.RecPJMsg)
            .WriteByte (message)
        End With
    End With
...
Public Sub WriteReceiveGetConsulta(ByVal UserIndex As Integer, ByVal Consulta As String)

On Error GoTo Errhandler
    
With UserList(UserIndex).outgoingData
    Call .WriteByte(ServerPacketID.ReceiveGetConsulta)
...
Public Sub WriteReceiveRespuestaConsulta(ByVal UserIndex As Integer)
On Error GoTo Errhandler

Call UserList(UserIndex).outgoingData.WriteByte(ServerPacketID.ReceiveRespuestaConsulta)
Call WriteConsoleMsg(UserIndex, "Servidor> Tu consulta ha sido respondida por un miembro del Staff; revisa tu casilla de consultas para ver la respuesta.", FontTypeNames.FONTTYPE_SERVER)
...
Public Sub WriteRespuestaGM(ByVal UserIndex As Integer, ByVal Consulta As String, ByVal Respuesta As String)
On Error GoTo Errhandler
    
With UserList(UserIndex).outgoingData
    Call .WriteByte(ServerPacketID.RespuestaGM)
    Call .WriteASCIIString(Consulta)
    Call .WriteASCIIString(Respuesta)
End With
        
UserList(UserIndex).flags.GMConsulta = vbNullString
UserList(UserIndex).flags.GMRespuesta = vbNullString
...
Public Sub HandleGetRespuestaGM(ByVal UserIndex As Integer)

With UserList(UserIndex)
    'Remove Packet ID
    Call .incomingData.ReadByte
    
    Call WriteRespuestaGM(UserIndex, .flags.GMConsulta, .flags.GMRespuesta)
End With

End Sub

Public Sub HandleGetConsulta(ByVal UserIndex As Integer)
If UserList(UserIndex).incomingData.length < 3 Then
    Err.Raise UserList(UserIndex).incomingData.NotEnoughDataErrCode
Exit Sub
...
With UserList(UserIndex)
    ...
    If .flags.Privilegios And (PlayerType.Admin Or PlayerType.Dios Or PlayerType.SemiDios) Then
        ...
        If NameIndex(UserName) <= 0 Then
            ...
            Call WriteConsoleMsg(UserIndex, "Servidor> Usuario offline, buscando consulta en charfile.", FontTypeNames.FONTTYPE_SERVER)
            
            If FileExist(CharPath & UserName & ".chr", vbArchive) = False Then
                Call WriteConsoleMsg(UserIndex, "Servidor> Personaje inexistente.", FontTypeNames.FONTTYPE_INFO)
            Else
                Call WriteReceiveGetConsulta(UserIndex, GetVar(CharPath & UserName & ".chr", "FLAGS", "GMConsulta"))
            End If
        Else
            Call WriteReceiveGetConsulta(UserIndex, UserList(NameIndex(UserName)).flags.GMConsulta)
        End If
   End If
...
Public Sub HandleResponderConsulta(ByVal UserIndex As Integer)
If UserList(UserIndex).incomingData.length < 3 Then
    Err.Raise UserList(UserIndex).incomingData.NotEnoughDataErrCode
Exit Sub
...
With UserList(UserIndex)
    ...
    If .flags.Privilegios And (PlayerType.Admin Or PlayerType.Dios Or PlayerType.SemiDios) Then
        ...
        If NameIndex(UserName) <= 0 Then
            ...
            Call WriteConsoleMsg(UserIndex, "Servidor> Usuario offline, guardando respuesta en charfile.", FontTypeNames.FONTTYPE_SERVER)
                                
            If FileExist(CharPath & UserName & ".chr", vbArchive) = False Then
                Call WriteConsoleMsg(UserIndex, "Pj Inexistente", FontTypeNames.FONTTYPE_INFO)
            Else
                If Len(GetVar(CharPath & UserName & ".chr", "FLAGS", "GMRespuesta")) = 0 Then
                    Call WriteVar(CharPath & UserName & ".chr", "FLAGS", "GMRespuesta", Respuesta)
                Else
                    Call WriteConsoleMsg(UserIndex, "Servidor> La consulta ya fue respondida.", FontTypeNames.FONTTYPE_SERVER)
                End If
            End If
        Else
            If Len(UserList(NameIndex(UserName)).flags.GMRespuesta) = 0 Then
                UserList(NameIndex(UserName)).flags.GMRespuesta = Respuesta
                WriteReceiveRespuestaConsulta (NameIndex(UserName))
            Else
                Call WriteConsoleMsg(UserIndex, "Servidor> La consulta ya fue respondida.", FontTypeNames.FONTTYPE_SERVER)
            End If
        End If
...
Public Sub HandleSolicitarCSU(ByVal UserIndex As Integer)
    On Error GoTo Errhandler
    ...
    With UserList(UserIndex)
        ...
        Nombre = buffer.ReadASCIIString
        tUser = NameIndex(Nombre)
        
        If Not EsGm(UserIndex) Then
            Call WriteConsoleMsg(UserIndex, "Comando inválido.", FONTTYPE_INFO)
        Else
            If tUser <= 0 Then
                Call WriteConsoleMsg(UserIndex, "Servidor> El usuario " & Nombre & " se encuentra offline.", FONTTYPE_INFO)
            Else
                Call WriteAskCSU(tUser)
            End If
        End If
...
Public Sub WriteAskCSU(ByVal tUser As Integer)

    With UserList(tUser)
        With .outgoingData
            Call .WriteByte(ServerPacketID.AskCSU)
        End With
    End With
    
End Sub

Public Sub HandleSendCSU(ByVal UserIndex As Integer)
On Error GoTo Errhandler
...
With UserList(UserIndex)
    ...
    Dim CSU As String
    Dim OriginalCSU As String
    
    CSU = buffer.ReadASCIIString
    OriginalCSU = GetVar(CharPath & UCase$(.Name) & ".chr", "INIT", "SecurityCode")
    
    If CSU <> OriginalCSU Then
        Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg("Servidor> " & .Name & " ha introducido el CSU incorrecto. Esta persona no es dueña del personaje.", FONTTYPE_INFO))
    Else
        Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg("Servidor> " & .Name & " ha introducido el CSU correcto. Se valida como dueño del personaje.", FONTTYPE_INFO))
    End If
...
Public Sub HandleCambiarPJ(ByVal UserIndex As Integer)
...
With UserList(UserIndex)
    ...
    Dim PersonajeUno As String
    Dim PersonajeDos As String
    Dim IDUno As Integer
    Dim IDDos As Integer
    
    Dim okUno As Boolean
    Dim okDos As Boolean
    
    Dim Count As Byte
    
    Dim PwPJUno As String
    Dim PwPJDos As String
    Dim EmailPJUno As String
    Dim EmailPJDos As String
    Dim CSUUno As String
    Dim CSUDos As String
    
    PersonajeUno = buffer.ReadASCIIString
    PersonajeDos = buffer.ReadASCIIString
    
    IDUno = NameIndex(PersonajeUno)
    IDDos = NameIndex(PersonajeDos)
    
    Dim Leer As clsIniManager
    Set Leer = New clsIniManager
 
    Call Leer.Initialize(CharPath & UCase$(PersonajeUno) & ".chr")
    
    PwPJUno = Leer.GetValue("INIT", "Password") 'Obtengo la contraseña del personaje 1
    EmailPJUno = Leer.GetValue("CONTACTO", "Email") 'Obtengo el email del personaje 1
    CSUUno = Leer.GetValue("INIT", "SecurityCode") 'Obtengo el CSU del personaje 1
    
    Call Leer.Initialize(CharPath & UCase$(PersonajeDos) & ".chr")
    
    PwPJDos = Leer.GetValue("INIT", "Password") 'Obtengo la contraseña del personaje 2
    EmailPJDos = Leer.GetValue("CONTACTO", "Email") 'Obtengo el email del personaje 2
    CSUDos = Leer.GetValue("INIT", "SecurityCode") 'Obtengo el CSU del personaje 2
    
    If Not EsGm(UserIndex) Then 'Si no es GM, muestro mensaje de error
        Call WriteConsoleMsg(UserIndex, "Comando inválido.", FONTTYPE_INFO)
    Else 'Si es GM...
        If (EsConsejero(.Name) = True) Or (EsSemiDios(.Name) = True) Or (EsRolesMaster(.Name) = True) Then 'Si es GM con jerarquía Consejero, Semi o RoleMaster no puede usar el comando
            Call WriteConsoleMsg(UserIndex, "Servidor> Tu jerarquía no es suficente para utilizar este comando.", FONTTYPE_INFO)
        Else 'Si es GM con jerarquía Dios o Admin
            If Not FileExist(CharPath & UCase$(PersonajeUno) & ".chr", vbNormal) Then 'Si no existe el personaje 1
                Call WriteConsoleMsg(UserIndex, "Servidor> El personaje" & PersonajeUno & " no existe.", FONTTYPE_INFO)
                okUno = False
            Else 'Si existe...
                If IDUno <= 0 Then 'Si el personaje 1 esta OFFLINE, le aviso
                    Call WriteConsoleMsg(UserIndex, "Servidor> El personaje" & PersonajeUno & " se encuentra offline.", FONTTYPE_INFO)
                    okUno = False
                Else 'Si esta ONLINE...
                    If (EsGm(IDUno) = True) Then 'Si el personaje 1 forma parte de los GMs activos y vos queres por algún motivo cambiar de personajes, el sistema Anti-Kidral sale a flote
                        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("MAB> " & .Name & " ha sido baneado por intentar intercambiar un GM con otro personaje.", FontTypeNames.FONTTYPE_DIOS))
                        Call CloseSocket(UserIndex)
                        Call WriteVar(CharPath & UCase$(PersonajeUno) & ".chr", "FLAGS", "Ban", 1)
                        
                        Count = val(GetVar(CharPath & UCase$(PersonajeUno) & ".chr", "PENAS", "Cant"))
                        Call WriteVar(CharPath & UCase$(PersonajeUno) & ".chr", "PENAS", "Cant", Count + 1)
                        Call WriteVar(CharPath & UCase$(PersonajeUno) & ".chr", "PENAS", "P" & Count + 1, "MAB: Ban por intentar cambiar un GM " & Date$ & " " & time$)
                        
                        okUno = False
                    Else 'Si no es un GM es un usuario normal (faltaría el check de concilio/consejo pero no vamos a tener)
                         If UserList(IDUno).GuildIndex > 0 Then 'Si el personaje 1 tiene clan...
                            If m_EsGuildFounder(UserList(IDUno).Name, UserList(IDUno).GuildIndex) Or m_EsGuildLeader(UserList(IDUno).Name, UserList(IDUno).GuildIndex) Then 'Si el personaje 1 es funda o líder no puede cambiar
                                Call WriteConsoleMsg(UserIndex, "Servidor> El personaje " & PersonajeUno & " es fundador o líder de clan.", FontTypeNames.FONTTYPE_SERVER)
                                okUno = False
                            Else 'Si no es funda ni lider, puede
                                okUno = True
                            End If
                         Else 'Si no tiene clan, puede
                            okUno = True
                         End If
                    End If
                End If
            ...
            If Not FileExist(CharPath & UCase$(PersonajeDos) & ".chr", vbNormal) Then 'Si no existe el personaje 2
                Call WriteConsoleMsg(UserIndex, "Servidor> El personaje" & PersonajeDos & " no existe.", FONTTYPE_INFO)
                okDos = False
            Else 'Si existe...
                If IDDos <= 0 Then 'Si el personaje 2 esta OFFLINE, le aviso
                    Call WriteConsoleMsg(UserIndex, "Servidor> El personaje" & PersonajeDos & " se encuentra offline.", FONTTYPE_INFO)
                    okDos = False
                Else 'Si esta ONLINE...
                    If EsGm(IDDos) = True Then 'Si el personaje 2 forma parte de los GMs activos y vos queres por algún motivo cambiar de personajes, el sistema Anti-Kidral sale a flote
                        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("MAB> " & .Name & " ha sido baneado por intentar intercambiar un GM. Ks1 prro.", FontTypeNames.FONTTYPE_DIOS))
                        Call CloseSocket(UserIndex)
                        Call WriteVar(CharPath & UCase$(PersonajeDos) & ".chr", "FLAGS", "Ban", 1)
                        
                        Count = val(GetVar(CharPath & UCase$(PersonajeDos) & ".chr", "PENAS", "Cant"))
                        Call WriteVar(CharPath & UCase$(PersonajeDos) & ".chr", "PENAS", "Cant", Count + 1)
                        Call WriteVar(CharPath & UCase$(PersonajeDos) & ".chr", "PENAS", "P" & Count + 1, "MAB: Ban por intentar cambiar un GM " & Date$ & " " & time$)
                        
                        okDos = False
                    Else 'Si no es un GM es un usuario normal (faltaría el check de concilio/consejo pero no vamos a tener)
                         If UserList(IDDos).GuildIndex > 0 Then 'Si el personaje 2 tiene clan...
                            If m_EsGuildFounder(UserList(IDDos).Name, UserList(IDDos).GuildIndex) Or m_EsGuildLeader(UserList(IDDos).Name, UserList(IDDos).GuildIndex) Then 'Si el personaje 2 es funda o líder no puede cambiar
                                Call WriteConsoleMsg(UserIndex, "Servidor> El personaje " & PersonajeDos & " es fundador o líder de clan.", FONTTYPE_INFO)
                                okDos = False
                            Else 'Si no es funda ni lider, puede
                                okDos = True
                            End If
            ...
            If (okUno = True) And (okDos = True) Then 'Si ambos son válidos para intercambiar, entonces procedemos
                If UserList(IDUno).GuildIndex > 0 Then 'Si el personaje 1 tiene clan, lo echo del mismo
                    Call modGuilds.m_EcharMiembroDeClan(-1, PersonajeUno)
                End If
                
                If UserList(IDDos).GuildIndex > 0 Then 'Si el personaje 2 tiene clan, lo echo del mismo
                    Call modGuilds.m_EcharMiembroDeClan(-1, PersonajeDos)
                End If
                
                Call CloseSocket(IDUno) 'Cierro la conexión del personaje 1
                Call CloseSocket(IDDos) 'Cierro la conexión del personaje 2
                
                'Cambios al personaje 1 *********************************
                Call Leer.Initialize(CharPath & UCase$(PersonajeUno) & ".chr")
                
                Call Leer.ChangeValue("INIT", "Password", PwPJDos)
                Call Leer.ChangeValue("CONTACTO", "Email", EmailPJDos)
                Call Leer.ChangeValue("INIT", "SecurityCode", CSUDos)
                
                Call Leer.DumpFile(CharPath & UCase$(PersonajeUno) & ".chr")
                '**************************************************
                
                'Cambios al personaje 2 *********************************
                Call Leer.Initialize(CharPath & UCase$(PersonajeDos) & ".chr")
                
                Call Leer.ChangeValue("INIT", "Password", PwPJUno)
                Call Leer.ChangeValue("CONTACTO", "Email", EmailPJUno)
                Call Leer.ChangeValue("INIT", "SecurityCode", CSUUno)
                
                Call Leer.DumpFile(CharPath & UCase$(PersonajeDos) & ".chr")
                
                Set Leer = Nothing
                '**************************************************
                
                Call WriteConsoleMsg(UserIndex, "Servidor> El cambio de personajes ha sido realizado con éxito.", FONTTYPE_INFO)
                
            Else 'Si por algún motivo ilógico (porque se supone que son válidos si llegaron hasta este IF) no son válidos, mostramos error
                Call WriteConsoleMsg(UserIndex, "Servidor> Ha ocurrido un error al intercambiar personajes. Por favor intente nuevamente más tarde.", FontTypeNames.FONTTYPE_SERVER)
            End If
...
Private Sub HandleKillAllNearbyNPCsWithRespawn(ByVal UserIndex As Integer)
        ...
        For X = .Pos.X - MinXBorder + 1 To .Pos.X + MinXBorder - 1
            ...
            If X > 0 And Y > 0 And X < 101 And Y < 101 Then
                If MapData(.Pos.Map, X, Y).NpcIndex > 0 Then
                    auxNPC = Npclist(MapData(.Pos.Map, X, Y).NpcIndex)
