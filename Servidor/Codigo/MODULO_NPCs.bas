...
Public Sub MuereNpc(ByVal NpcIndex As Integer, ByVal UserIndex As Integer)
                        ...
                        If MiNPC.flags.ExpCount > 0 Then
                                ...
                                Else
                                        ...
                                        If .Stats.Exp > MAXEXP Then _
                                           .Stats.Exp = MAXEXP
                                        Call WriteConsoleMsg(UserIndex, "Has ganado " & IIf(MiNPC.flags.ExpCount > 999, FormatNumber(MiNPC.flags.ExpCount, 0, vbFalse, vbFalse, vbTrue), MiNPC.flags.ExpCount) & " puntos de experiencia.", FontTypeNames.FONTTYPE_FIGHT)
        ...
        If MiNPC.GiveGLD > 0 Then
            Call WriteConsoleMsg(UserIndex, "Has ganado " & IIf(MiNPC.GiveGLD > 999, FormatNumber(MiNPC.GiveGLD, 0, vbFalse, vbFalse, vbTrue), MiNPC.GiveGLD) & " monedas de oro.", FontTypeNames.FONTTYPE_DIOS)
        End If
                ...
                Do While Not PosicionValida
                        ...
                        If Iteraciones > MAXSPAWNATTEMPS Then
                                If altpos.X <> 0 And altpos.Y <> 0 Then
                                        ...
                                        'MAB: Ahora trigger 3 es posición inválida & antirespawn
                                        If MapData(Map, X, Y).trigger <> eTrigger.POSINVALIDA Then
                                ...
                                Else
                                        ...
                                        If newpos.X <> 0 And newpos.Y <> 0 Then
                                                ...
                                                'MAB: Ahora trigger 3 es posición inválida & antirespawn
                                                If MapData(newpos.Map, newpos.X, newpos.Y).trigger <> eTrigger.POSINVALIDA Then
...
Public Sub MakeNPCChar(ByVal toMap As Boolean, _
        ...
        If Not toMap Then
                Call WriteCharacterCreate(sndIndex, Npclist(NpcIndex).Char.Body, Npclist(NpcIndex).Char.Head, Npclist(NpcIndex).Char.heading, Npclist(NpcIndex).Char.CharIndex, X, Y, Npclist(NpcIndex).Char.WeaponAnim, Npclist(NpcIndex).Char.ShieldAnim, 0, 0, Npclist(NpcIndex).Char.CascoAnim, vbNullString, 0, 0, Npclist(NpcIndex).Stats.MinHp, Npclist(NpcIndex).Stats.MaxHP)
